...
<section id={nodeId} class="performance-growth" bind:this={el}>
	...
	<div class="growth-rings" role="img" aria-label="四项指标同比增幅，圆环使用0%到80%的统一刻度">
		...
		{#each growth as item, index}
			<article style={`--progress:${Math.min(item.value / scaleMax, 1) * 360}deg; --ring:${index};`}>
...
<style>
	...
	.total-ticker div { padding:.9rem .8rem; }
	...
	.growth-rings article { display:grid; justify-items:center; align-content:start; min-width:0; text-align:center; }
	.ring { width:min(132px,100%); aspect-ratio:1; display:grid; place-items:center; border:1.5px solid var(--panel-border); border-radius:50%; background:conic-gradient(var(--panel-accent) 0 var(--progress), color-mix(in srgb,var(--panel-border) 12%,transparent) var(--progress) 360deg); box-shadow:4px 4px 0 color-mix(in srgb,var(--panel-border) 18%,transparent); }
	.ring-2 { background:conic-gradient(var(--panel-accent-2) 0 var(--progress), color-mix(in srgb,var(--panel-border) 12%,transparent) var(--progress) 360deg); }
	.ring-3 { background:conic-gradient(color-mix(in srgb,var(--panel-accent) 42%,var(--panel-accent-2) 58%) 0 var(--progress), color-mix(in srgb,var(--panel-border) 12%,transparent) var(--progress) 360deg); }
	.ring-4 { background:conic-gradient(color-mix(in srgb,var(--panel-text) 58%,var(--panel-accent-2) 42%) 0 var(--progress), color-mix(in srgb,var(--panel-border) 12%,transparent) var(--progress) 360deg); }
